...
class Laporan extends CI_Controller
{
    ...
    function laporan_spp()
    {
        if (isset($_GET['filter']) && !empty($_GET['filter'])) {
            ...
            if ($filter == '1') {
                ...
                $nisn = $_GET['nisn'];
                $ket = 'Data Transaksi dari Siswa dengan Nomor Induk ' . $nisn;
                $url_cetak = 'laporan/cetak2?&nisn=' . $nisn;
                $spp_bulanan = $this->M_transaksi->view_by_nisn($nisn)->result();
            } else {
                $tahun = $_GET['tahun'];
        ...
        $data['nisn'] = $this->M_transaksi->nisn();
    ...
    public function cetak2()
    {
        ...
        ob_start();
        require('assets/fpdf/fpdf.php');
        $data['spp_bulanan'] = $this->M_transaksi->view_by_nisn($nisn)->result();
        $data['ket'] = $ket;
        $this->load->view('laporan/cetak_spp', $data);
    }
    ...
    function laporan_buku()
    {
        if (isset($_GET['filter']) && !empty($_GET['filter'])) {
            ...
            if ($filter == '1') {
                ...
                $nisn = $_GET['nisn'];
                $ket = 'Data Transaksi dari siswa dengan Nomor Induk ' . $nisn;
                $url_cetak = 'laporan/cetak97?&nisn=' . $nisn;
                $pem_lainya = $this->M_transaksi->nisn_buku($nisn)->result();
            } else {
                $tahun = $_GET['tahun'];
                $ket = 'Data Transaksi Tahun Ajaran ' . $tahun;
                $url_cetak = 'laporan/cetak4?&tahun=' . $tahun;
            ...
            $pem_lainya = $this->M_transaksi->laporan_buku();
        }
        $data['ket'] = $ket;
        $data['url_cetak'] = base_url($url_cetak);
        ...
        $data['nisn'] = $this->M_transaksi->nisn();
        $data['tahun'] = $this->M_transaksi->tahun();
        $data['title'] = 'Laporan Data Pembayaran Buku';
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
    ...
    public function cetak97()
    {
        ...
        ob_start();
        require('assets/fpdf/fpdf.php');
        $data['pem_lainya'] = $this->M_transaksi->nisn_buku($nisn)->result();
        $data['ket'] = $ket;
        $this->load->view('laporan/cetak_lainya', $data);
    }
