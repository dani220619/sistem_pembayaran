...
class Snap extends CI_Controller
{
	...
	public function token()
	{
		...
		//echo json_encode($transaction_data);exit;
	...
	public function cekStatusTransaksi($idtransaksi, $nisn, $orderid)
	{
		try {
			$response = $this->midtrans->status($orderid);
			if ($response->status_code == 200) {
				$sql = "UPDATE spp_bulanan SET status='0' WHERE id_transaksi='$idtransaksi' AND nisn='$nisn' AND order_id='$orderid'";
				$this->db->query($sql);
				echo 'success';
			} else {
				if ($response->transaction_status == 'pending') {
				} else {
					$sql = "UPDATE spp_bulanan SET status='1' WHERE id_transaksi='$idtransaksi' AND nisn='$nisn' AND order_id='$orderid'";
					$this->db->query($sql);
				}
